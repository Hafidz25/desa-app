...
export default class KartuKeluarga extends Component {
  constructor(props) {
    super(props);
    this.state = {
      access_token: "",
  ...
  _GetDataWarga = async () => {
    ...
    // let url = "http://10.0.0.151:8000/api/v1/kks?limit=0";
  ...
  SearchFilterFunction(text) {
    const newData = this.arrayholder.filter(function (item) {
      ...
      const itemData1 = item.legal_name
        ? item.legal_name.toUpperCase()
        : "".toUpperCase();
  ...
  deleteData = async (id) => {
    ...
    // let url = "http://10.0.0.151:8000/api/v1/kks/"+id;
    let url = "https://host02.birosolusi.com/edesa/public/api/v1/kks/" + id;
  ...
  renderItem = ({ item, index }, onClick) => {
    ...
    return (
      ...
      >
        <TouchableOpacity
          onPress={() =>
            this.props.navigation.navigate("Detail Data Kartu Keluarga", {
              id: item.id,
            })
          }
        >
            ...
            <View style={styles.userBg}>
              <View style={styles.menuItem}>
                <Text style={styles.menuItemText}>
                  {/* {this.state.dataPenduduk.map((t)=> {return t.legal_name})} */}
                  {item.head.penduduk.legal_name}
                </Text>
              </View>
              <View style={styles.menuItem}>
                <Text style={styles.bgText}>No. KK : {item.kk}</Text>
              </View>
              <View style={styles.jkStyle}>
                <Text style={styles.jkText}></Text>
              </View>
            </View>
          </View>
        </TouchableOpacity>
      ...
      <View style={styles.container}>
          ...
          <View style={styles.searchContainer}>
            ...
            >
              <Searchbar
                fontSize={13}
                iconColor="#444"
                placeholder="Cari Data Warga By No. KK atau Nama"
        ...
        <View style={{ marginTop: 10, marginBottom: 10, padding: 10 }}>
          <View>
            <Text style={styles.tittleText}>
              Jumlah Warga {this.state.data.length}
            </Text>
          </View>
        </View>
        <FlatList
        ...
        <View>
          <TouchableOpacity
            ...
            activeOpacity={0.5}
            onPress={() =>
              this.props.navigation.replace("Tambah Kartu Keluarga")
            }
          >
